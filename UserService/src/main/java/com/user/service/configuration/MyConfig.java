...
public class MyConfig {
	...
	@Bean
	@LoadBalanced //By this we can call services using name of service 
	public RestTemplate restTemplate()
	{
		RestTemplate restTemplate = new RestTemplate();
		List<ClientHttpRequestInterceptor>interceptors=new ArrayList();
		//here we can set list of interceptor manually in resttemplate
		//but here we have only one interceptor
		interceptors.add(new RestTemplateInterceptor(
				manager(clientRegistrationRepository,clientRepository)));		
//		for adding interceptor we have to create interceptors
		restTemplate.setInterceptors(interceptors);
		return restTemplate;
	}
	
	//declaring the bean of authorizedclientmanager
	@Bean
